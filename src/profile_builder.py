...
class ValveProfileBuilder:
    """
    Deterministic helper around ValveProfile schema:
    ...
    def apply_patch(self, profile: Dict[str, Any], patch: Dict[str, Any]) -> List[str]:
        ...
        for k, raw in patch.items():
            if k not in self.props:
                errors.append(f"Unknown field '{k}' (not in ValveProfile schema)")
                continue
    ...
    def render_markdown(self, profile: Dict[str, Any], title: str = "ValveProfile") -> str:
        """
